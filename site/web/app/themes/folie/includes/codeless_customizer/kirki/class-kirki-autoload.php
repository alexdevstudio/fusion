...
class Kirki_Autoload {
	...
	protected function get_paths( $class_name ) {
		...
		if ( isset( $name_parts[0] ) && 'Modules' === $name_parts[0] ) {
			// codeless
			$path  = CODELESS_KIRKI_PATH . '/modules/';
		...
		if ( isset( $name_parts[0] ) ) {
			...
			if ( 'Control' === $name_parts[0] || 'Settings' === $name_parts[0] ) {
				// codeless
				$path  = CODELESS_KIRKI_PATH . '/controls/php/';
				$paths[] = $path . $filename;
			}
		}
		// codeless
		$paths[] = CODELESS_KIRKI_PATH . '/core/' . $filename;
		$paths[] = CODELESS_KIRKI_PATH . '/lib/' . $filename;
		...
		for ( $i = 0; $i < $levels; $i++ ) {
			// codeless
			$paths[] = CODELESS_KIRKI_PATH . '/' . $previous_path . strtolower( $exploded[ $i ] ) . '/' . $filename;
